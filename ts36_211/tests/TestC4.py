...
from ..c4 import DwPTS, UpPTS, GP, T_slot, T_s, symbol_nr_DwPTS, symbol_nr_UpPTS
...
class TestC4(unittest.TestCase):
    ...
    def test_UpPTS(self):
        ...
        for ul_cp in CP_TYPE.all():
            for ssp in SPECIAL_SUBFRAME_PATTERNS.all():
                eq_(symbol_nr_UpPTS(ssp, dl_cp, ul_cp), (1,1,1,1,1,2,2,2,2)[ssp-SPECIAL_SUBFRAME_PATTERNS.SSP0])
        dl_cp = CP_TYPE.EXTENDED
        for ul_cp in CP_TYPE.all():
            for ssp in SPECIAL_SUBFRAME_PATTERNS.all()[:7]:
                eq_(symbol_nr_UpPTS(ssp, dl_cp, ul_cp), (1,1,1,1,2,2,2)[ssp-SPECIAL_SUBFRAME_PATTERNS.SSP0])